...
const loginPayLoad = {userEmail: "sravanthich2021@gmail.com", userPassword: "Myfirstjob@2021"};
let token;
test.beforeAll(  async()=>
{
    ...
    const Login_Response = await this.APIContext.post("https://rahulshettyacademy.com/api/ecom/auth/login",
             ...
             expect(Login_Response.ok()).toBeTruthy(); // for success response 200/201/202 etc
             const Login_Response_json = await Login_Response.json(); // to get the response json
             token=Login_Response_json.token; // to get the token from the response json
             const status = Login_Response.status();
             console.log(token);
...
test('E2E Testing,async',async ({page})=>
  {
   ...
   // await page.waitForLoadState('networkidle'); 
    ...
    const products = page.locator(".card-body");// list the all the products // Home page
