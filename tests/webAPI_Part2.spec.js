...
test.beforeAll(async({browser})=>
{
    ...
    await context.storageState({path : 'state.json'}) // we have to store all the session information at browser level, not page level, so we write like this
   // one json file will be created at test level with the above statement.
   // Invoke a browser by injecting state.json file
   webContext = await browser.newContext({storageState: 'state.json'}); // new context means new browser having all the knowledge of what we have done so far
})
test('E2E Testing,async',async()=>{
    //const context = await browser.newContext(); // to create a fresh browser without plugins and cookies,only browser instance created
    //const page = await context.newPage(); // new page will be created ,with in created instance
    //const products = page.locator(".card-body");// list the all the products
    const prodcutName = 'ADIDAS ORIGINAL';
   ...
   const products = page.locator(".card-body");// list the all the products
   await page.goto("https://rahulshettyacademy.com/client/"); // login url,here there is no login credentials
