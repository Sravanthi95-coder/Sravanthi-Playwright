...
test(`E2E Testing for ${data.productName}`, async ({ browser }) => {
   ...
   await loginpagePO.validLogin(data.username, data.password);
   //********************************************************************************************

   const dashBoardPOobj = new dashBoardPO(page); // created object for dashBoardPO file
   await dashBoardPOobj.searchProduct_AddCart(data.productName);
   await dashBoardPOobj.navigateToCart();
   // *********************************************************************************************
   const cartPagePOobj = new cartPagePO(page);
   await cartPagePOobj.verifyProductDisplayed(data.productName);
